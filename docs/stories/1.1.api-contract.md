...
- 新增 `pkg/signerapi` HTTP/gRPC handler + Buf 生成/`make api-ci` 流程，接入 digest 校验与 5ms SLA 测试
...
- pkg/signerapi/backend.go
- pkg/signerapi/http.go
- pkg/signerapi/http_test.go
- pkg/signerapi/grpc.go
- pkg/signerapi/grpc_test.go
# QA Results
...
- 2025-11-20 Quinn (QA) —— Gate: PASS
  ...
  - [实现] 新增 `pkg/signerapi` HTTP/gRPC handler 接入 `validator` 与 `apierrors`，确保 `/sign` 仅接受 32B digest，错误统一映射；`make api-ci` 串联 Buf lint/generate、stub 差异校验、SLA/单测回归（pkg/signerapi/http.go:1-200，pkg/signerapi/grpc.go:1-63，Makefile:1-23）。
  - [测试] `make api-ci` ✅（含 `buf lint`、`buf generate --template buf.gen.yaml`、`go test ./pkg/signerapi -run TestCreateFastPathBudget`、`go test ./pkg/... ./docs/api/tests`）。
